...
df_clear %>%
  group_by(Make) %>%
  summarise(sum_price = sum(Price)) %>%  arrange(desc(sum_price))
...
df_clear %>%  select(Price , everything())


df_clear %>%  select(-Price)
...
df_clear  %>% rename(Model = Make , Odometer..KM.) %>%  select(Model) %>%  filter(Model == 'Nissan') 
